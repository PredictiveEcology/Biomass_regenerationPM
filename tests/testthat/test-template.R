...
#    `functionName` is a function's name in your module (e.g., `Biomass_regenerationPMEvent1`).
# 2. Copy this file to the tests folder (i.e., `E:/GitHub/LandscapesInMotion/R/SpaDES/m/Biomass_regenerationPM/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Biomass_regenerationPM")
  ...
  parameters <- list(
    ...
    Biomass_regenerationPM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Biomass_regenerationPMEvent1", envir = .GlobalEnv)) {
    simOutput <- Biomass_regenerationPMEvent1(mySim)
  } else {
  ...
  if (exists("Biomass_regenerationPMEvent2", envir = .GlobalEnv)) {
    simOutput <- Biomass_regenerationPMEvent2(mySim)
  } else {
